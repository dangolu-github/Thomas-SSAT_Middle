(function () {
  ...
  function bindCard(card){var item=items[Number(card.dataset.reviewItem)],msg=card.querySelector('[data-card-status]'),content=card.querySelector('[data-question-content]'),form=card.querySelector('[data-redo-form]'),answer=card.querySelector('[data-answer]'),toggle=card.querySelector('[data-answer-toggle]'),detail=null,loading=null;
    ...
    toggle.addEventListener('click',function(){if(!answer.hidden){hideAnswer();return;}form.hidden=true;busy(true);msg.textContent='正在读取答案…';request('mistakeReviewAnswer',ref(item)).then(function(r){if(pageRedo&&!pageRedo.finished){hideAnswer();return;}answer.innerHTML='<p>原作业选择：<strong>'+esc(r.firstChoice||'未作答')+'</strong></p><p>正确答案：<strong>'+esc(r.correctAnswer)+'</strong> '+esc(r.correctText)+'</p>'+(r.reasoning?'<p>'+esc(r.reasoning)+'</p>':'')+(r.optionNotes||[]).map(function(n){return '<p>'+esc(n)+'</p>';}).join('');answer.hidden=false;toggle.textContent='隐藏答案';toggle.setAttribute('aria-expanded','true');msg.textContent='';}).catch(function(e){error(msg,e);}).finally(function(){busy(false);});});
    var action={item:item,card:card,form:form,hideAnswer:hideAnswer,
      begin:function(){hideAnswer();msg.textContent='';form.querySelector('[data-redo-options]').innerHTML='ABCDE'.split('').map(function(letter){return '<label class="review-choice"><input type="radio" name="redo-'+Number(card.dataset.reviewItem)+'" value="'+letter+'"><span>'+letter+'</span></label>';}).join('');form.hidden=false;card.open=true;busy(false);},
      result:function(r){item.redoCount=r.redoCount;item.lastRedo={correct:r.correct};msg.textContent=r.correct?'本页重做：答对了。':'本页重做：还需复习。';var history=content.querySelector('.review-history');if(history)history.textContent='已重做 '+r.redoCount+' 次 · 最近一次：'+(r.correct?'答对':'还需复习');},
      unlock:function(){busy(false);}
    };cardActions.push(action);form.addEventListener('change',updatePageControls);
  ...
  function updatePageControls(){var active=!!pageRedo,working=active&&!pageRedo.finished;
    ...
    if(pageRedo.finished)return;
    var answered=pageRedo.cards.filter(function(a){return a.form.querySelector('input:checked');}).length;
    if(!pageRedo.checking)redoStatus.textContent='本页 '+pageRedo.cards.length+' 题 · 已作答 '+answered+' 题。完成后统一检查本页答案。';
  }
  startRedo.addEventListener('click',function(){if(!cardActions.length)return;pageRedo={cards:cardActions.slice(),requestId:window.crypto.randomUUID(),results:{},checking:false,finished:false};list.querySelectorAll('.review-group').forEach(function(g){g.open=true;});pageRedo.cards.forEach(function(a){a.begin();});checkRedo.textContent='检查本页答案';updatePageControls();});
  cancelRedo.addEventListener('click',function(){if(!pageRedo||pageRedo.checking)return;pageRedo=null;cardActions.forEach(function(a){a.form.hidden=true;a.hideAnswer();a.unlock();});updatePageControls();});
  checkRedo.addEventListener('click',function(){if(!pageRedo||pageRedo.checking)return;var run=pageRedo;
    var missing=run.cards.find(function(a){return !a.form.querySelector('input:checked');});
    if(missing){redoStatus.textContent='本页还有未作答的题目，请完成后再统一检查。';missing.card.open=true;missing.card.closest('.review-group').open=true;missing.form.querySelector('input').focus();return;}
    run.checking=true;updatePageControls();run.cards.forEach(function(a){a.form.querySelectorAll('input').forEach(function(input){input.disabled=true;});});
    var next=0,failed=0;function worker(){if(pageRedo!==run)return Promise.resolve();var n=next++;if(n>=run.cards.length)return Promise.resolve();if(run.results[n])return worker();var a=run.cards[n];
      redoStatus.textContent='正在检查本页答案：'+Object.keys(run.results).length+' / '+run.cards.length+' 题…';
      return request('mistakeReviewRedo',Object.assign(ref(a.item),{choice:a.form.querySelector('input:checked').value,requestId:run.requestId+'-'+n})).then(function(r){run.results[n]=r;}).catch(function(){failed++;}).then(worker);
    }
    Promise.all([worker(),worker(),worker()]).then(function(){if(pageRedo!==run)return;run.checking=false;
      if(failed){updatePageControls();checkRedo.textContent='重试检查本页答案';redoStatus.textContent='已检查 '+Object.keys(run.results).length+' / '+run.cards.length+' 题，连接暂时中断。答案已保留，请重试完成本页检查。';return;}
      run.finished=true;var correct=0;run.cards.forEach(function(a,n){var r=run.results[n];if(r.correct)correct++;a.result(r);a.unlock();});updatePageControls();redoStatus.textContent='本页重做完成：'+correct+' / '+run.cards.length+' 题答对。原作业成绩保持不变。';
    });
  });
